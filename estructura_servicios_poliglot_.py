...
def consultar_gemini(prompt, temperatura=0.7):
    ...
    try:
        ...
        respuesta = gemini_client.models.generate_content(
            ...
            model="gemini-2.5-flash-lite",  # Modelo rápido y económico
            contents=prompt,
...
# ==================== FUNCIÓN PRINCIPAL CON HILOS ====================
def generar_campana_completa(descripcion_producto, mercado, llm_seleccionado="todos", idioma_entrada="es"):
    """
    Esta es nuestra función más importante, ahora con HILOS para paralelizar las llamadas a IAs.
    ...
    if llm_seleccionado == "todos":
        llms_a_usar = list(consultas_disponibles.keys())  # ["deepseek", "mistral", "gemini"]
        modo = "comparativa"
    elif llm_seleccionado in consultas_disponibles:
        llms_a_usar = [llm_seleccionado]
        modo = "single"
    else:
    ...
    # Inicializar estructura de contenido
    for tipo in prompts.keys():
        resultados["contenido"][tipo] = {}
    
    # ==================== HILOS: PROCESAR EN PARALELO ====================
    print(f"\n🚀 [HILOS] Iniciando generación paralela para {len(llms_a_usar)} LLMs...")
    tiempo_inicio_total = time.time()
    
    # Usar ThreadPoolExecutor para ejecutar en paralelo
    with ThreadPoolExecutor(max_workers=len(llms_a_usar)) as executor:
        futuros = {}
        
        # Para cada LLM, crear una tarea que generará los 3 tipos de contenido
        for llm_nombre in llms_a_usar:
            consulta_func = consultas_disponibles[llm_nombre]
            futuro = executor.submit(
                procesar_llm_completo,
                llm_nombre,
                consulta_func,
                prompts,
                mercado,
            ...
            futuros[futuro] = llm_nombre
        
        # Recolectar resultados a medida que terminan
        for futuro in as_completed(futuros):
            llm_nombre = futuros[futuro]
            try:
                contenido_llm = futuro.result(timeout=60)
                # Integrar resultados al diccionario principal
                for tipo, datos in contenido_llm.items():
                    resultados["contenido"][tipo][llm_nombre.capitalize()] = datos
                print(f"✅ [HILOS] {llm_nombre.capitalize()} completado")
            except Exception as e:
                print(f"❌ [HILOS] Error en {llm_nombre}: {str(e)}")
                # Registrar error para este LLM
                for tipo in prompts.keys():
                    resultados["contenido"][tipo][llm_nombre.capitalize()] = {
                        "respuesta": None,
                        "traduccion": None,
                        "exito": False,
                        "error": str(e),
    ...
    print(f"✅ [HILOS] Generación completa en {tiempo_total:.0f} ms")
    
    return resultados


def procesar_llm_completo(llm_nombre, consulta_func, prompts, mercado, idioma_entrada):
    """
    Función auxiliar que se ejecuta en un hilo separado.
    Procesa los 3 tipos de contenido para un LLM específico.
    """
    codigo_idioma = MERCADOS[mercado]["codigo_idioma"]
    contenido_llm = {}
    
    for tipo_contenido, prompt in prompts.items():
        inicio_tipo = time.time()
        
        # Para eslóganes de Mistral, usar función especial
        if tipo_contenido == "eslogans" and llm_nombre == "mistral":
        ...
        contenido_llm[tipo_contenido] = {
            "respuesta": respuesta_texto,
            "traduccion": traduccion,
            "exito": respuesta.get("exito"),
            "error": respuesta.get("error"),
            "tiempo_ms": respuesta.get("tiempo_ms", int((time.time() - inicio_tipo) * 1000))
        }
    
    return contenido_llm

# ==================== FUNCIÓN PARA COMPARAR LLMS ====================
